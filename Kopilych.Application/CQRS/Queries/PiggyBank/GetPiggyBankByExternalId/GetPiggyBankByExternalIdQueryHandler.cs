...
{
    public class GetPiggyBankByExternalIdQueryHandler : IRequestHandler<GetPiggyBankByExternalIdQuery, PiggyBankDTO>
    {
        ...
        public GetPiggyBankByExternalIdQueryHandler(IPiggyBankRepository repository, IMapper mapper, IUserInfoService userInfoService, IPiggyBankService piggyBankService)
            => (_repository, _mapper, _userInfoService, _piggyBankService) = (repository, mapper, userInfoService, piggyBankService);
        public async Task<PiggyBankDTO> Handle(GetPiggyBankByExternalIdQuery request, CancellationToken cancellationToken)
        {
            var piggybank = await _repository.GetByExternalIdAsync(request.ExternalId, cancellationToken);
            var hideInfo = true;
            if (piggybank == null)
            {
                throw new NotFoundException(nameof(piggybank), request.ExternalId);
            }
